...
import dict_ver2_jv8.DictionaryCommandLine;
import dict_ver2_jv8.Word;
...
public class test {
	...
	JButton insertBtn; // line 403
	JButton deleteBtn; // line 488
	JButton addBtn; // line 320
	// search line 188
	//  button search 252
	// pronunciation 279
	...
	private void initialize() throws IOException {
		dictcmd.getDictMNG().insertFromFile();// nhap du lieu tu file khi khoi dong 
		...
		frmFrame.addWindowListener( new WindowListener() { // truoc khi dong thi ghi lai toan bo tu vao file
			...
			public void windowClosing(WindowEvent e) {
				...
				try {			
					dictcmd.getDictMNG().exportToFile();
		...
		comboBox.getEditor().getEditorComponent().addKeyListener(new KeyAdapter() { // su kien phim cua search box
			...
			public void keyReleased(KeyEvent e) {
				...
				if( chckbxUseGoogleTranslate.isSelected() ) {
					 //su dung google
					if(!x.equals("") ) {
						//System.out.println("sử dựng google");
					...
					if( !x.equals("")) {
						if(e.getKeyCode() == e.VK_ENTER  ) {
							Word word_target = dictcmd.getDictMNG().dictionaryLookup(x);
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// goi searcher va lookup
				String x = comboBox.getEditor().getItem().toString();
				if( !x.equals("") ) { // o tim kiem khac rong
					if( dictcmd.getDictMNG().containTarget(x) ) { // neu tu co ton tai thi in ra nghia
						Word word_target = dictcmd.getDictMNG().dictionaryLookup(x);
						viewBox.setText("Từ : "+ word_target.getWord_target()+ "   /" + word_target.getWord_pronoun() +"/" );
						viewBox.setText(viewBox.getText() + "\nNghĩa\n" + word_target.getWord_explain() );
					}
		...
		addBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				// goi cua so them tu
				//System.out.println("them tu"); // check
				//addframe.run(); 
				// goi cua so them tu
				...
				enterBtn.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						//statusLabel.setText("enter được click");
						...
						if( target.equals("") || pronoun.equals("") || explain.equals("")) { 
							...
							String status = dictcmd.getDictMNG().addData(target, pronoun, explain);
							statusLabel.setText(status);
						}
						//dictcmd.showAllWord();
					}
				...
				enterBtn.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if( target.equals("") || pronoun.equals("") || explain.equals("")) { 
							...
							String status = dictcmd.getDictMNG().insertData(target, pronoun, explain);
							statusLabel.setText(status);
						}
						//dictcmd.showAllWord();
					}
				...
				btnNewButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if( target.equals(""))
						{
							...
							String status = dictcmd.getDictMNG().deleteData(target);
							statusLabel.setText(status);
						}			
						//dictcmd.showAllWord();
					}
